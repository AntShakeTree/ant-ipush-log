...
import com.ant.ipush.kafka.delivery.AsynchronousDeliveryStrategy;
...
/**
 * 该类中的属性均会通过logback配置文件进行初始化 （getter/setter）
 *
 *     <appender name="KafkaAppender" class="com.ant.ipush.kafka.KafkaAppender">
 *         <encoder>
 *             <pattern>%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] xxx- %msg</pattern>
 *         </encoder>
 *
 *         <topic>${topic}-${active}</topic>
 *         <keyingStrategy class="com.ant.ipush.keying.EventKeyingStrategy"/>
 *         <deliveryStrategy class="com.ant.ipush.kafka.delivery.AsynchronousDeliveryStrategy"/>
 *
 *         <producerConfig>bootstrap.servers=${kafkaSrv}</producerConfig>
 *         <producerConfig>bootstrap.aa=${kafkaSrv}</producerConfig>
 *     </appender>
 * @since 0.0.1
 */
